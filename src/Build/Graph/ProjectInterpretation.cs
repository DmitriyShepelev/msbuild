...
namespace Microsoft.Build.Graph
{
    internal sealed class ProjectInterpretation
    {
        ...
        private static readonly IImmutableSet<string> NonskippableTargets = ImmutableHashSet.Create(MSBuildConstants.DefaultTargetsMarker, MSBuildConstants.ProjectReferenceTargetsOrDefaultTargetsMarker);
        ...
        private readonly record struct TargetSpecification
        {
            /// <summary>
            /// Initializes a new instance of the <see cref="TargetSpecification"/> record struct.
            /// </summary>
            /// <param name="target">The target.</param>
            /// <param name="skipIfNonexistent"><c>true</c> iff the <paramref name="target"/> can
            /// be skipped if it is nonexistent.</param>
            public TargetSpecification(string target, bool skipIfNonexistent)
            {
                ErrorUtilities.VerifyThrow(!skipIfNonexistent || !NonskippableTargets.Contains(target), target + " cannot be marked as SkipNonexistentTargets");
                Target = target;
        ...
        public readonly struct TargetsToPropagate
        {
            ...
            public ImmutableList<string> GetApplicableTargetsForReference(ProjectInstance reference)
            {
                ...
                {
                    // Keep non-skipped targets or targets that exist but are skippable.
                    return targets
